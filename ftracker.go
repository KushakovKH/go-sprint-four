...
const (
	...
	runningCaloriesMeanSpeedShift      = 1.79 // среднее количество сжигаемых калорий при беге1.79.
)
...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	runWalk := meanSpeed(action, duration)
	runSpCal := ((runningCaloriesMeanSpeedMultiplier * runWalk * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return runSpCal
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	speedWalking := meanSpeed(action, duration) * kmhInMsec
	speedWalkingSquared := math.Pow(speedWalking, 2)
	heightMet := height / cmInM
	walkSpCal := (((walkingCaloriesWeightMultiplier * weight) + ((speedWalkingSquared / heightMet) * walkingSpeedHeightMultiplier * weight)) * duration * minInH)
	return walkSpCal
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	meanSpeedPoll := swimmingMeanSpeed(lengthPool, countPool, duration)
	swimSpCal := (meanSpeedPoll + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return swimSpCal
}
